...
export default function EditContractPage() {
  ...
  useEffect(() => {
    Promise.all([
      apiFetch(`/organizations/${orgSlug}/contracts/${contractId}`),
    ...
    ]).then(([contract, c, co, p, o, u]) => {
      setForm({
        title: contract.title || '', contractType: contract.contractType || 'SERVICE_AGREEMENT', status: contract.status || 'DRAFT',
        currency: contract.currency || 'USD', contractValue: contract.contractValue?.toString() || '',
        paymentTerms: contract.paymentTerms || '', autoRenewal: contract.autoRenewal || false, notes: contract.notes || '',
        startDate: contract.startDate ? contract.startDate.split('T')[0] : '', endDate: contract.endDate ? contract.endDate.split('T')[0] : '',
        renewalDate: contract.renewalDate ? contract.renewalDate.split('T')[0] : '',
        companyId: contract.companyId || '', contactId: contract.contactId || '', proposalId: contract.proposalId || '',
        opportunityId: contract.opportunityId || '', ownerId: contract.ownerId || '',
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiFetch(`/organizations/${orgSlug}/contracts/${contractId}`, { method: 'PATCH', body: JSON.stringify(payload) });
      router.push(`/${orgSlug}/crm/contracts/${contractId}`);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold tracking-tight">Edit Contract</h1><p className="text-sm text-gray-500 mt-1">Update contract details</p></div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="border border-gray-200 dark:border-zinc-800 rounded-lg bg-white dark:bg-zinc-900 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              ...
              <input type="text" name="paymentTerms" value={form.paymentTerms} onChange={handleChange} className="w-full rounded-md border border-gray-200 dark:border-zinc-700 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
            </div>
            <div className="sm:col-span-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select name="proposalId" value={form.proposalId} onChange={handleChange} className="w-full rounded-md border border-gray-200 dark:border-zinc-700 bg-transparent px-3 py-2 text-sm">
                <option value="">None</option>{proposals.map((p) => <option key={p.id} value={p.id}>{p.title}</option>)}
              </select>
            </div>
            <div>
        ...
        <div className="flex items-center justify-end gap-3">
          <Link href={`/${orgSlug}/crm/contracts/${contractId}`} className="px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-md">Cancel</Link>
