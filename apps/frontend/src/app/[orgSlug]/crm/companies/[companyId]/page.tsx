...
import {
  Building2, Globe, Mail, Phone, MapPin, Users, DollarSign,
  ArrowLeft, Edit, Trash2, Plus, Calendar, Tag, FileText,
...
export default function CompanyDetailPage() {
  ...
  const fetchCompany = async () => {
    try {
      const data = await apiFetch(`/organizations/${orgSlug}/companies/${companyId}`);
      setCompany(data);
    } catch (err) {
  ...
  const handleAddNote = async () => {
    ...
    setAddingNote(true);
    try {
      await apiFetch(`/organizations/${orgSlug}/companies/${companyId}/notes`, {
        method: 'POST',
  ...
  const handleDelete = async () => {
    try {
      await apiFetch(`/organizations/${orgSlug}/companies/${companyId}`, { method: 'DELETE' });
  ...
  if (!company) {
    return (
      <div className="text-center py-20">
        ...
        <Link href={`/${orgSlug}/crm/companies`} className="text-primary hover:underline text-sm mt-2 inline-block">
          Back to Companies
        </Link>
      </div>
    );
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                {company.companyType && (
                  <span className="text-sm text-gray-400">{company.companyType}</span>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2">
          <Link
            href={`/${orgSlug}/crm/companies/${companyId}/edit`}
              ...
              <div className="space-y-3">
                ...
                {company.website && (
                  <div className="flex items-center gap-3 text-sm">
                    ...
                    <a href={company.website.startsWith('http') ? company.website : `https://${company.website}`} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline inline-flex items-center gap-1">
                  ...
                  <div className="flex items-start gap-3 text-sm">
                    ...
                    <div>
                      ...
                      {[company.city, company.postalCode, company.country].filter(Boolean).join(', ') && (
                        <p>{[company.city, company.postalCode, company.country].filter(Boolean).join(', ')}</p>
                      )}
                    </div>
                  </div>
                )}
            ...
            <div className="border border-gray-200 dark:border-zinc-800 rounded-lg bg-white dark:bg-zinc-900 p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {company.assignedTo ? (
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-sm font-medium">{company.assignedTo.firstName} {company.assignedTo.lastName}</p>
                        <p className="text-xs text-gray-500">{company.assignedTo.email}</p>
                      </div>
                    </div>
                  ) : (
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-full bg-gray-100 dark:bg-zinc-800 flex items-center justify-center text-xs font-bold">
                      {company.createdBy.firstName?.[0]}{company.createdBy.lastName?.[0]}
                    </div>
                    <div>
                      <p className="text-sm font-medium">{company.createdBy.firstName} {company.createdBy.lastName}</p>
                ...
                <div>
                  ...
                  <p className="text-sm">{new Date(company.updatedAt).toLocaleDateString()}</p>
                </div>
              </div>
            </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-lg max-w-md w-full p-6">
            <h3 className="text-lg font-bold mb-2">Delete Company</h3>
