...
export default function EditLeadPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setForm({
        ...
        tagsInput: lead.tags?.map((t: any) => t.name).join(', ') || '',
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiFetch(`/organizations/${orgSlug}/leads/${leadId}`, {
        method: 'PATCH',
        body: JSON.stringify(payload),
      });

      router.push(`/${orgSlug}/crm/leads/${leadId}`);
    } catch (err: any) {
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="border border-gray-200 dark:border-zinc-800 rounded-lg bg-white dark:bg-zinc-900 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" name="companyName" value={form.companyName} onChange={handleChange} className="w-full rounded-md border border-gray-200 dark:border-zinc-700 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
            </div>
            <div>
        ...
        <div className="flex items-center justify-end gap-3">
          <Link href={`/${orgSlug}/crm/leads/${leadId}`} className="px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-md transition-colors">Cancel</Link>
